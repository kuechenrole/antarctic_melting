...
% # <http://www.mathworks.com/matlabcentral/fileexchange/42353.html Bedmap2> is not a perfect dataset by any means, but has fast 
% become the most widely-used surface, bed, and ice thickness dataset for Antarctica.  Depending on what kind of work you do, you 
% may find Bedmap2 useful, even if only as a common reference dataset.  
% 
...
%  help plotps 
% 
% displays plain-text help in the Command Window.  For formatted documentation with images, the 
% |showdemo| command can be used with any function name, followed by the |_documentation| suffix. 
% For example, 
% 
%  showdemo plotps_documentation
...
% 
% For example, a Bedmap2 plugin enables access to Bedmap2's 1-km resolution bed elevation, surface elevation, and ice thickness 
% datasets.  If you want to load enough Bedmap2 bed elevation data to fill the map we made above, the |bedmap2_data| 
...
[lat,lon,bed] = bedmap2_data('bed',xlim,ylim); 

%% 
% Then use <pcolorps_documentation.html |pcolorps|> to plot the Bedmap2 bed elevation as a pseudocolor
% plot: 
...
%% 
% Instead of importing the Bedmap2 data via the |bedmap2_data| function then plotting with |pcolorps|, 
% we could have just used the |bedmap2| plotting function, which can plot Bedmap2 data as |pcolor| or 
% contour data.  Here are some red contour lines corresponding to every 500 m of surface elevation:  

bedmap2('surfc','z',500:500:5000,'r','xy')

%% 
% One nice thing about AMT is the way it lets you bring multiple datasets together.  For example, plot InSAR-derived surface velocities 
% with the MEaSUREs plugin: 

measuresps('velocity','color','black')
...
%% 
% With the 150 m resolution coordinates of the straight line from South Pole to Taylor Dome, we can use |bedmap2_interp|
...
%% 
% But you know, it just so happens that the Bedmap2 Toolbox has a fancy function called |bedmap2_profile|, 
% which is made specifically for plotting elevation profiles along a transect: 

subplot(2,1,1)
bedmap2_profile(lati,loni)

%% 
% To see the Bedmap2 profile in context with ice surface velocities, use |measures_interp|: 

speed = measures_interp('speed',lati,loni); 
...
% how you might calculate ice volume flow rates by pairing MEaSUREs velocity data with Bedmap2 
% ice thickness data.  See the |measures_interp| documentation for an example of calculating 
