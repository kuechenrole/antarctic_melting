...
grd_vars = [grd_vars, 'zice', 'hraw'];               % needs to be last
...
if (Imin == 1),
  IpC = half+Gfactor:Gfactor:L;
  IrC = 1   :Gfactor:L;
  IuC = half+Gfactor:Gfactor:L;
  IvC = 1   :Gfactor:L;
else
...
if (Jmin == 1),
  JpC = half+Gfactor:Gfactor:M;
  JrC = 1   :Gfactor:M;
  JuC = 1   :Gfactor:M;
  JvC = half+Gfactor:Gfactor:M;
else
...
if (got.zice),
    status = nc_write (Gout, 'zice', C.zice);
end
