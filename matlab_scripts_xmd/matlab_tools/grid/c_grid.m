...
%    case {'zice'}
%      Dsize.(field) = 0; %netcdf.getConstant('UNLIMITED');
  end
end
...
%  if (~gotDim.zice), Dsize.zice = nc_constant('nc_unlimited'); end
...
%  Model ice shelf draft at RHO-points.

if (~gotVar.zice),
  Var.name          = Vname.zice;
  Var.type          = netcdf.getConstant('nc_double');
  Var.dimid         = [did.eta_rho did.xi_rho];
  Var.long_name     = 'model ice draft at RHO-points';
  Var.units         = 'meter';
  [~,status] = nc_vdef(ncid,Var);
  if (status ~= 0), return, end
  clear Var
