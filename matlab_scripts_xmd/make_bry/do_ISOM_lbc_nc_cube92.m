...
sstart = 1;
send = Lp;
angle_south=repmat(angle(1,:)',[1,31,12*(MaxYear-MinYear+1)]);
...
estart=Lp+1;
eend=Lp+Mp;
angle_east=repmat(angle(:,end),[1,31,12*(MaxYear-MinYear+1)]);
...
nstart=Lp+Mp+1;
nend=Lp+Mp+Lp;
angle_north=repmat(angle(end,:)',[1,31,12*(MaxYear-MinYear+1)]);
...
wstart=Lp+Mp+Lp+1;
wend=Lp+Mp+Lp+Mp;
angle_west=repmat(angle(:,1),[1,31,12*(MaxYear-MinYear+1)]);
